...
let read_variables buf count =
  ...
  let rec read_variables' count index result =
    if count > 0 then
      ...
      in
      let (v : Variable.t) =
        { index; name; name2; value_type; initval; location = None }
      in
  ...
  let rec read_globals' count index result =
    if count > 0 then
      ...
      let (variable : Variable.t) =
        { index; name; name2; value_type; initval = None; location = None }
      in
  ...
  let rec read_function_types' count index result =
    if count > 0 then
      ...
      let (ft : FunctionType.t) =
        { index; name; return_type; nr_arguments; variables; location }
      in
...
let rec data_to_string ain = function
  ...
  | IFaceWrap no ->
      sprintf "interface_wrap<%d>" no (* FIXME: look up name in ain object *)
  | Function no | Method no ->
      if no >= 0 then function_to_string_hum ain ain.functions.(no)
      else "function"
  | NullType -> "null"
...
and function_to_string_hum ain (f : Function.t) =
  sprintf "%s %s(%s)"
    (type_to_string_hum ain f.return_type)
    f.name
    (List.map (List.take f.vars f.nr_args) ~f:(variable_to_string_hum ain)
    |> String.concat ~sep:", ")
...
let set_global_type_loc ain name t loc =
  ...
  | Some (i, g) ->
      Array.set ain.globals i
        {
          g with
          variable = { g.variable with value_type = t; location = Some loc };
        }
...
let get_filename ain no = ain.filenames.(no)
